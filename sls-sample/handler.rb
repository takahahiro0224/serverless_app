...
class DynamoTable
  ...
  def self.update_item(id:, body:)
    ...
    [:title, :desc, :status, :is_done].each do |k|
      update_opts[k] = body[k.to_s] if body[k.to_s]
    end
    update(update_opts)
  end

  def self.start_item(id:)
    update({id: id, status: 1, updated_at: Time.now.to_i})
  end

  def self.done_item(id:)
    update({id: id, status: 2, updated_at: Time.now.to_i})
  end
